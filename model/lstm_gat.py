...
class SimpleGATWithStatic3Hop(nn.Module):
    ...
    def __init__(self, temporal_dim, static_dim, hidden_dim, output_dim, num_heads=4, dropout=0.5):
        ...
        self.conv1 = GATConv(in_channels=combined_dim, out_channels=hidden_dim // num_heads, heads=num_heads, dropout=dropout, concat=True)
        
        self.conv2 = GATConv(in_channels=hidden_dim, out_channels=hidden_dim // num_heads, heads=num_heads, dropout=dropout, concat=True)
        
        self.conv3 = GATConv(in_channels=hidden_dim, out_channels=hidden_dim // num_heads, heads=num_heads, dropout=dropout, concat=True)
...
class CombinedLSTMGATWithStatic3Hop(nn.Module):
    ...
    def forward(self, dynamic_features, static_features, edge_index):
        ...
        dynamic_feat_reshaped = dynamic_features.reshape(batch_size * n_stations, *dynamic_features.shape[2:])
        static_feat_reshaped = static_features.reshape(batch_size * n_stations, -1)
        
        station_embeddings = self.lstm_encoder(dynamic_feat_reshaped, static_feat_reshaped)
        
        station_embeddings = station_embeddings.reshape(batch_size, n_stations, -1)
