...
def prepare_data_for_mode(use_returns=False, seq_len=30, horizon=7):
    """
    Prepara los datos y calcula las estadísticas de normalización 
    específicas según si el modelo opera con precios absolutos o retornos.
    """
...
def compare_models_performance(models_preds_real, tgts_real, tickers, dates_list, horizon):
    ...
    colors = {
        'saved_models_h7_FF': 'tab:purple',   # Modelo tradicional (Precios en niveles)
        'saved_models_h1_TT_NR': 'tab:blue',  # Modelo nuevo (Retornos + Loss Híbrida)
    }
    ...
    plt.suptitle("Comparativa: Niveles (0.7) vs Retornos + Loss Direccional (Saved Models) - Horizonte t+1", fontsize=14, fontweight='bold', y=0.98)
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    # =========================================================================
    # CASO 1: Evaluar Modelo Tradicional (saved_models_h7_FF) -> USE_RETURNS = False
    # =========================================================================
    print("\n--- Procesando entorno para Modelo Tradicional (Niveles) ---")
    v_set_f, edge_idx_f, dates_list, mean_f, std_f, prices_data_f = prepare_data_for_mode(use_returns=False, seq_len=seq_len, horizon=horizon)
    dyn_f, stat_f, tgts_f = v_set_f
    dyn_f, stat_f, edge_idx_f = dyn_f.to(device), stat_f.to(device), edge_idx_f.to(device)
    
    if tgts_f.ndim == 3 and tgts_f.shape[-1] == 1:
        tgts_f = tgts_f.squeeze(-1)
        
    mean_arr_f = mean_f.values.reshape(1, len(tickers), 1)
    std_arr_f = std_f.values.reshape(1, len(tickers), 1)
    
    # Target real en USD para el modelo tradicional
    tgts_real = tgts_f.numpy() * std_arr_f + mean_arr_f

    # Cargar e inferir modelo tradicional
    model_path_trad = "saved_models_h7_FF/best_model.pth"
    print(f"Cargando modelo tradicional desde '{model_path_trad}'...")
    model_trad = CombinedLSTMGATWithStatic3Hop(
        dynamic_input_dim=1, static_input_dim=3, lstm_hidden_dim=32, gnn_hidden_dim=32, output_dim=horizon
    ).to(device)
    model_trad.load_state_dict(torch.load(model_path_trad, map_location=device, weights_only=True))
    model_trad.eval()

    with torch.no_grad():
        preds_trad = model_trad(dyn_f, stat_f, edge_idx_f).cpu().numpy()
    
    models_preds_real['saved_models_h7_FF'] = preds_trad * std_arr_f + mean_arr_f


    # =========================================================================
    # CASO 2: Evaluar Modelo Nuevo (saved_models_h7_TT) -> USE_RETURNS = True
    # =========================================================================
    print("\n--- Procesando entorno para Modelo con Retornos y Loss Híbrida ---")
    v_set_t, edge_idx_t, _, mean_t, std_t, prices_data_t = prepare_data_for_mode(use_returns=True, seq_len=seq_len, horizon=horizon)
    ...
    # Cargar e inferir modelo nuevo
    model_path_new = "saved_models_h7_TT/best_model.pth"
    print(f"Cargando modelo con retornos desde '{model_path_new}'...")
    model_new = CombinedLSTMGATWithStatic3Hop(
        dynamic_input_dim=1, static_input_dim=3, lstm_hidden_dim=32, gnn_hidden_dim=32, output_dim=horizon
    ).to(device)
    model_new.load_state_dict(torch.load(model_path_new, map_location=device, weights_only=True))
    model_new.eval()

    with torch.no_grad():
        preds_new = model_new(dyn_t, stat_t, edge_idx_t).cpu().numpy()

    # Reconstrucción de predicciones a precios USD acumulativos (Corregido)
    model_preds_usd = np.zeros_like(preds_new)
    ...
    for i, d_date in enumerate(dates_list):
        ...
        denorm_rets = preds_new[i] * std_arr_t + mean_arr_t
    ...
    models_preds_real['saved_models_h7_TT'] = model_preds_usd

    # 4. Lanzar comparación unificada
